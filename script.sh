...
get_hypervisor | tee "$_OUT"

for _ID in ${_IDS[@]}; do 
get_status 2&> /dev/null
	if [ $? -eq 0 ]; then echo -e "${_HEAD}> Container ${_SUCC}${_ID}${_END}" | tee -a "$_OUT"
		echo | tee -a "$_OUT"
		get_hostname | tee -a "$_OUT"
		get_os | tee -a "$_OUT"
		get_network | tee -a "$_OUT"
		echo | tee -a "$_OUT"
	elif [ $? -eq 1 ]; then 
		if [ ! -z "$(cat /etc/pve/lxc/${_ID}.conf | grep arch)" ]; then
			echo -e "${_HEAD}> Container ${_WARN}${_ID}${_END}" | tee -a "$_OUT"
			echo | tee -a "$_OUT"
			echo -e -n "${_WARN}Warning:${_END} Container ${_WARN}${_ID}${_END} has an issue: "| tee -a "$_OUT"
			pct start "${_ID}"
		else
			echo -e "${_HEAD}> Container ${_FAIL}${_ID}${_END}" | tee -a "$_OUT"
			echo | tee -a "$_OUT"
			echo -e -n "${_FAIL}Error:${_END} Container ${_FAIL}${_ID}${_END} is invalid: " | tee -a "$_OUT"
			pct start "${_ID}"
		fi
		echo | tee -a "$_OUT"
	else
		echo -e "${_HEAD}> Container ${_FAIL}${_ID}${_END}" | tee -a "$_OUT"
		echo | tee -a "$_OUT"
		echo -e "${_FAIL}Error:${_END} Container ${_FAIL}${_ID}${_END} not found!" | tee -a "$_OUT"
		echo | tee -a "$_OUT"
	fi
done 
